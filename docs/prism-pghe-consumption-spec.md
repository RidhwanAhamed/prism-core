...
- **Envelopes.** Raised-cosine attack (40–250 ms from `sharpness`) and release (1.5 × attack); if attack + release exceed `pulse_s` both scale down so the pulse is a pure bump. Per-frame amplitude delta is therefore bounded by the fastest attack at the ceiling: sampling `0.5·(1 − cos(πt/T))` with `T = 40 ms` every 10 ms gives steps of 0.146, 0.354, 0.354, 0.146 of the peak, i.e. at most `0.60 × 0.354 ≈ 0.21`; the continuous-slope bound `0.60 · π / (2 · 4 frames) ≈ 0.24` is what `PgheRender.NoAmplitudeSteps` asserts, with margin. (An audio-style 1e-3 bound is meaningless at 100 Hz; the haptic statement of "nothing steps" is: every rise is a raised cosine of ≥ 4 frames, every fall ≥ 6 frames, and no frame-to-frame change exceeds that slope.)
